#include "../../ngx_http_waf_module/inc/ngx_http_waf_module_config.h"
...
//    if (loc_conf->waf == 1) { modified 删掉此处是因为，当waf不启用时也需要加载相关信息，endpoint指令会使它启动
	if (ngx_http_waf_alloc_memory(cf, loc_conf) != NGX_HTTP_WAF_SUCCESS) {
		return NGX_CONF_ERROR;
	}
	//modified
	ngx_str_t *value,*vl;
	ngx_http_waf_loc_conf_t	**ccf;
	value = cf->args->elts;
	for ( size_t i = 0 ; i < waf_env_conf.count ; i++ ) {
		vl = waf_env_conf.name->elts;
		vl = &vl[i];
		if( vl->len == value[2].len &&
				ngx_strncmp( vl->data, value[2].data, value[2].len )== 0) {
			ngx_conf_log_error(NGX_LOG_ERR, cf, 0, "ngx_waf: %s", "waf directive must has one name");
			return NGX_CONF_ERROR;
		}
	}
	vl = ngx_array_push(waf_env_conf.name);
	vl->data = value[2].data;
	vl->len = value[2].len;
	ccf = ngx_array_push(waf_env_conf.waf_cf);
	*ccf = loc_conf;
	waf_env_conf.count++;
        //
//    }
...
char* ngx_http_waf_rule_path_conf(ngx_conf_t* cf, ngx_command_t* cmd, void* conf) {
    ...
    if (ngx_http_waf_load_all_rule(cf, loc_conf,NULL) != NGX_HTTP_WAF_SUCCESS) {
        return NGX_CONF_ERROR;
    }
...
void* ngx_http_waf_create_main_conf(ngx_conf_t* cf) {
    ...
    //modified
    waf_env_conf.count = 0;
    waf_env_conf.name = ngx_array_create(cf->pool,1,sizeof(ngx_str_t));
    waf_env_conf.waf_cf = ngx_array_create(cf->pool,1,sizeof(ngx_http_waf_loc_conf_t*));
    return main_conf;
}
