#include "../../ngx_http_waf_module/inc/ngx_http_waf_module_core.h"
...
//modified
ngx_int_t ngx_http_waf_valid(ngx_http_waf_loc_conf_t *cf)
{
	ngx_http_waf_loc_conf_t *pcf;
	pcf = cf->parent;
	while(pcf != NULL) {
		cf = pcf;
		pcf = cf->parent;
	}
	return cf->waf != 0 && cf->waf != NGX_CONF_UNSET ;
}
