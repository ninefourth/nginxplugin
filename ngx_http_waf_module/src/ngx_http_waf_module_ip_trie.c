...
//modified
ngx_int_t _ip_trie_clear(mem_pool_t *pool , ip_trie_node_t* node,uint8_t fg) {
	uint8_t ret = NGX_HTTP_WAF_SUCCESS;
	//
	if( node != NULL) {
		if( node->left != NULL) {
			ret = _ip_trie_clear(pool,node->left,1);
		}
		if( node->right != NULL) {
			ret = _ip_trie_clear(pool,node->right,1);
		}
		//
		if(ret == NGX_HTTP_WAF_SUCCESS) {
			if (node->data != NULL){
				ret = mem_pool_free(pool, node->data);
				node->data_byte_length = 0;
				node->data = NULL;
			}
			if(fg ==1 ) {
				ret = mem_pool_free(pool, node);
			} else {
				node->left = NULL;
				node->right = NULL;
			}
...
ngx_int_t ip_trie_clear(ip_trie_t* trie) {
	ip_trie_node_t	*root = trie->root;
	if(root != NULL) {
		return _ip_trie_clear(&trie->pool,root,0);
	}
	return NGX_HTTP_WAF_SUCCESS;
}
